...
export default function OwnProfile() {
  ...
  return (
    <div className="own-profile-shown">
      <img className="pfp-own-profile" src={userInfo.picture} alt="" />
      <h1 className="pfp-content">{userInfo.name}</h1>
      <h3 className="pfp-content">{userInfo.email}</h3>
      <div className="bio-container"><button type="button" className="profile-button" onClick={updateBio}>Update Bio</button></div>
      { shouldUpdateBio ? (
        <div className="update-bio-container">
          <input className="new-bio" type="text" placeholder="enter new bio info..." ref={bioReference} />
          <button type="button" className="profile-button" onClick={sendBio}>Submit Bio</button>
        </div>
      ) : null}
      <div className="follow-container"><button type="button" className="profile-button" onClick={updateFollow}>Update Followed Events</button></div>
      { shouldUpdateFollow ? (
        <div className="update-follow-container">
          <form className="follow-form" onSubmit={sendFollow}>
            <MultiSelect
              options={options}
              value={selected}
      ...
      <button type="button" className="profile-button" onClick={onUpdateInterestsClicked}>Update Interests</button>
